...
void PPG::drawPPGCurve(Adafruit_SH1106G& display) {
    ...
    constexpr int manualAmplitude = 1200;
    const int windowSize = min(BUFFER_SIZE, 60);
    ...
    for (int i = 0; i < windowSize; ++i) {
        const int idx = (baseIdx + i) % BUFFER_SIZE;
    ...
    if (autoZoomEnabled) {
        ...
        displayMin = -manualAmplitude;
        displayMax = manualAmplitude;
    }
