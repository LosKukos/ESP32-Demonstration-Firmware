...
void SnakeLib::web_SnakeLib() { // Registrace HTTP koncových bodů pro Snake hru
    ...
    web.server.on("/snake", HTTP_GET, [this](AsyncWebServerRequest *request) { // Koncový bod pro načtení hlavní stránky hry Snake
        request->send_P(200, "text/html", snakePage); // Odeslání HTML stránky pro hru Snake
    });

    web.server.on("/snake/setDifficulty", HTTP_GET, [this](AsyncWebServerRequest *request) { // Koncový bod pro nastavení obtížnosti hry Snake přes HTTP požadavky
        if (request->hasParam("level")) { // Kontrola, zda je v HTTP požadavku přítomen argument "level" pro nastavení obtížnosti
            int level = request->getParam("level")->value().toInt(); // Převedení hodnoty argumentu "level" na celé číslo pro nastavení obtížnosti
            setDifficulty(level); // Volání funkce pro nastavení obtížnosti hry Snake podle zadané hodnoty argumentu "level"
        }
        request->send(200, "text/plain", "Difficulty set"); // Odeslání odpovědi pro potvrzení nastavení obtížnosti
    });

    web.server.on("/snake/setName", HTTP_GET, [this](AsyncWebServerRequest *request) { // Koncový bod pro nastavení jména hráče pro záznam
        if (request->hasParam("name")) { // Kontrola, zda je v HTTP požadavku přítomen argument "name" pro nastavení jména hráče
            String name = request->getParam("name")->value(); // Uložení zadaného jména hráče
        ...
        request->send(200, "text/plain", "OK"); // Odeslání odpovědi pro potvrzení nastavení jména hráče
    });

    web.server.on("/snake/leaderboard", HTTP_GET, [this](AsyncWebServerRequest *request) { // Koncový bod pro načtení tabulky s výsledky hráčů pro zobrazení na webu
    ...
    request->send(200, "application/json", json); // Odeslání JSON pole s výsledky hráčů pro zobrazení na webu
    });

    web.server.on("/snake/exit", HTTP_GET, [this](AsyncWebServerRequest *request) { // Koncový bod pro opuštění Snake a návrat do hlavního menu přes HTTP požadavky
