...
void SnakeLib::begin() {
    ...
    if (taskHandle == nullptr) {
        ...
        if (taskHandle == nullptr) {
            running = false;
            Serial.println("[SNAKE] static task create FAIL");
            return;
        }

        Serial.println("[SNAKE] static task create OK");
    } else {
        vTaskResume(taskHandle);
        Serial.println("[SNAKE] task resumed");
    }
}
...
void SnakeLib::task(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (self->_exit) {
            ...
            Serial.printf("[SNAKE] task suspended | stack free minimum: %u bytes\n",
...
void SnakeLib::renderGameOver() {
    ...
    controls.mutexDisplay([&]() {
        ...
        controls.display.setCursor(0, 42);
        controls.display.println("Web: uloz jmeno ted");

        controls.display.setCursor(0, 54);
        controls.display.println("Touch = menu / ztrata");
...
void SnakeLib::spawnFruit() {
    ...
    const int fruitMinX = GRID_MIN_X + FRUIT_MARGIN;
    const int fruitMaxX = GRID_MAX_X - FRUIT_MARGIN;
    const int fruitMinY = GRID_MIN_Y + FRUIT_MARGIN;
    const int fruitMaxY = GRID_MAX_Y - FRUIT_MARGIN;
...
void SnakeLib::clearSnakeRoutes() {
    ...
    web.server().on("/snake/clearLeaderboard", HTTP_GET, [this](AsyncWebServerRequest *request) {
